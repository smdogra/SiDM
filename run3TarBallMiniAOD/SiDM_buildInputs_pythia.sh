...
for FILE in pythiafragments/BsTo2DpTo4Mu_MBs-500_MDp-5p0_ctau-80p0.py
do
    echo  $FILE
    ...
    new_file_name=$(echo "${dirname}" | sed 's/\(.*\)_ctau.*/\1/')
    echo ${new_file_name}
    ...
    sed -i "s/processname/${new_file_name}/"  inputs/${dirname}.py


    echo "TARBALL=${new_file_name}_el9_amd64_gcc11_CMSSW_13_2_9_tarball.tar.xz" > ./submit/inputs.sh
    echo "HADRONIZER=${dirname}.py  " >> ./submit/inputs.sh
	...
	mkdir -p ./submit/input/
	cp ${TARBALLDIR}/inputs/${new_file_name}_el9_amd64_gcc11_CMSSW_13_2_9_tarball.tar.xz ./submit/input/
	cp ${TARBALLDIR}/inputs/${dirname}.py ./submit/input/
	...
	cp ${TARBALLDIR}/inputs/${new_file_name}_el9_amd64_gcc11_CMSSW_13_2_9_tarball.tar.xz ./submit/input/
	cp ${TARBALLDIR}/inputs/${dirname}.py ./submit/input/
	cp ${BASEDIR}/exec2024.sh $SUBMIT_WORKDIR
    fi
